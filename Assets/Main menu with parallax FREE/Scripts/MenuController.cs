...
public class MenuController : MonoBehaviour {
    ...
    public int option { get; private set; } = 0;
    ...
    private bool OptionIsActivated = false; //option창이 활성화되어 있으면 화살표 버튼 안 눌리게 할 것 
    ...
    public UnityAction[] Events = new UnityAction[3];
    ...
    void Start()
    {
        ...
        if (useParallax) { activeBackground = new GameObject[backgroundsParallax.Length]; }
        else { activeBackground = new GameObject[backgrounds.Length]; }
        initiate();
        Events[0] += newGame;
        Events[1] += openOptions;
        Events[2] += exitGame;
    }
    ...
    {
        if(!OptionIsActivated && option < options.Length-1)
        {
            option = option + 1;
    ...
    public void newGame()
    {
        ...
        if (1 == GameManager.GetInstance.Stage)
            GameManager.GetInstance.LoadPrologue();
        else
    ...
    {
        OptionIsActivated = false;
        menuText.raycastTarget = true;
